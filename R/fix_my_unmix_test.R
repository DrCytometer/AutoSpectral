...
#' @title Fix My Unmix (Test Version)
#'
#' @description
...
#'   `get.af.basis.library()`. When `NULL` and `bg.mode = "af.deconv"`, it is
#'   built from `af.spectra` (computed internally via `get.af.spectra()` with
#'   its own defaults, if not already supplied) and `spectra`. Default `NULL`.
#' @param af.n.pc Unused when `bg.mode = "af.deconv"`;
#'   `get.af.basis.library()` is called with its own defaults
#'   (`n.components = NULL`, `var.explained = 0.99`). Retained for backward
#'   compatibility. Default `"auto"`.
...
#'   with at least two rows, when `bg.mode = "per.cell"`. When
#'   `bg.mode = "af.deconv"` and `af.basis` is `NULL`, supplying this skips
#'   the internal `get.af.spectra()` call. Unused otherwise.
  ...
  background.basis <- switch(
    ...
    af.deconv = {
      if ( is.null( af.basis ) ) {
        ...
        af.basis <- t( get.af.basis.library( af.spectra, spectra.fluor )$basis )
      }
      as.matrix( af.basis )
    },
    af.row = {
