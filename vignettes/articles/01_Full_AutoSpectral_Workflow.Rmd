...
Once, the control file passes the error checks, we can create some gates.
...
Now we can choose which samples we want to use to define the gate boundaries. The simplest option is to just use everything. To do this, put "TRUE" in every row of the `gate.define` column. This should be done by default in version 1.5.0 and higher.
...
Note that you can create any number of gates and pass them in the next step. Gates are saved and can be reused. See the dedicated articles on this for more information, including on how to use the `tune.gate()` function.
...
![Unstained dead cell gate](figures/Workflow/AF Negative dead.jpg)

![Unstained myeloid gate](figures/Workflow/AF negative myeloid.jpg)
