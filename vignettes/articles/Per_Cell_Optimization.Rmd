...
What we are going to do is similar to what we did with the single cell AF extraction. We will start by mapping the variation in the single-colour controls for each fluorophore using a self-organizing map (SOM). 
...
Set `parallel` to `TRUE` for faster processing with `futures` if you have enough memory available. This should take a couple of minutes.
