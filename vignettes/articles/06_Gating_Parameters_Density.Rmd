...
For better control of the gating, update to AutoSpectral version 1.5.0, and use the `tune.gates()` function to get the gates looking the way you want before calling `define.flow.control()`. For this, see article [Tuning Gates]().
...
Today we're going to use a simple PBMC set run on the FACSDiscover A8. The FACSDiscover cytometers are, I think, a nice example for today because they have a very wide working range for the scatter detectors, which, while great for dealing with a huge array of sample types, makes the automated gating more challenging due to the large search space. You can try this out by downloading some files from the [OMIP-102 dataset](), or better, try it out on your own data (be sure to set the correct cytometer).
...
Immediately we have a failure, at least with this version of AutoSpectral. You may have run into this sort of issue, with incomprensible error messages, which is really frustrating. The new approach (see article here) should make this a lot more manageable, but let's work through it here as well.
