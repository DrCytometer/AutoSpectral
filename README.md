...
packages separately. As of version 0.9.1, AutoSpectral relies primarily
on `FlowSOM` rather than `EmbedSOM`, so you will need to install
`FlowSOM` via Bioconductor. That said, if you install `EmbedSOM`,
Autospectral will automatically detect and use it for faster processing.
