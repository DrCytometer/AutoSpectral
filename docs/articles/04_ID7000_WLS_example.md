...
Below is a basic example of a non-AutoSpectral unmixing workflow, using
samples from the ID7000. This only illustrates weighted least squares
unmixing. For per-cell autofluorescence extraction or per-cell
fluorophore optimization, see the articles on those topics. The point of
this workflow is to show how the control clean-up and automated
determination of fluorophore spectra can mimic or improve upon the
unmixing obtained on the Sony using their WLSM unmixing. The workflow
here *just* uses weighted least squares (WLS) for unmixing.
