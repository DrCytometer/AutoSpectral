...
want before calling
[`define.flow.control()`](https://drcytometer.github.io/AutoSpectral/reference/define.flow.control.md).
For this, see article [Tuning
Gates](https://drcytometer.github.io/AutoSpectral/articles/).
...
Today we’re going to use a simple PBMC set run on the FACSDiscover A8.
The FACSDiscover cytometers are, I think, a nice example for today
because they have a very wide working range for the scatter detectors,
which, while great for dealing with a huge array of sample types, makes
the automated gating more challenging due to the large search space. You
can try this out by downloading some files from the [OMIP-102
dataset](https://drcytometer.github.io/AutoSpectral/articles/), or
better, try it out on your own data (be sure to set the correct
cytometer).
...
incomprensible error messages, which is really frustrating. The new
approach (see article here) should make this a lot more manageable, but
let’s work through it here as well.
